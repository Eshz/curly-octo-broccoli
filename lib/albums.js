...
export const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || "4523";
...
export function getAdminSession(req, res) {
  res.status(200).json({ authenticated: verifyAdminSession(getCookie(req, ADMIN_SESSION_COOKIE)) });
}

export function requireAdmin(req, res) {
  if (verifyAdminSession(getCookie(req, ADMIN_SESSION_COOKIE))) return true;
...
export function extractEventDate(html) {
  ...
  // Unix ms timestamp (13 digits, e.g. 1778744864000)
  ...
  // Unix seconds timestamp (10 digits, e.g. 1778760317)
